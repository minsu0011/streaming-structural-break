"""Separate production entry for the reviewed serial-variance registered bank."""
import sys
...
from src.next.serial_variance_runtime import PreparedSerialVariancePredictor, source_hash as runtime_hash
...
def load_artifact(directory):
    ...
    if descriptor.get('kind') != 'NEXT_REGISTERED_BINARY' or descriptor.get('runtime') != 'serial_variance_fast_h':
        raise RuntimeError('Explicit reviewed serial-variance artifact required')
    ...
    if candidate.get('extension', {}).get('kind') != 'serial_variance':
        raise RuntimeError('This production adapter accepts only the reviewed serial-variance family')
...
def infer(datasets, model_directory_path):
    ...
    prepared = PreparedSerialVariancePredictor(model)
...
def train(datasets, model_directory_path):
    from src.next.serial_training import train_frozen_candidate
    path = Path(__file__).resolve().parents[2]/'configs/next_serial_deployment.json'
    if not path.is_file():
