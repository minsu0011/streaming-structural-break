"""Explicit artifact for historical AR8 plus frozen conditional variance input."""
from dataclasses import dataclass
from pathlib import Path
import joblib
from src.features.arch_input import ARCHCalibratedState,implementation_hash
...
@dataclass
class ARCHCalibratedBundle:
    base_model:object
    policy:object=CalibrationPolicy(method='historical_median_mad')
    feature_config:object=CONFIG
    feature_hash:str=''
    @property
    def name(self):return self.base_model.name+'_ar8_arch1'
    def make_state(self,historical):return ARCHCalibratedState(historical,config=self.feature_config,policy=self.policy)
    ...
    def load(path):
        ...
        if model.feature_hash!=implementation_hash(model.policy,model.feature_config):raise RuntimeError('ARCH representation changed')
